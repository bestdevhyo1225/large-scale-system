...
CQRS 패턴을 적용한 `Command, Query` 모듈 서버에서는 `231.8 TPS` 의 쓰기 작업, `10723.4 TPS` 의 읽기 작업이 처리된다. 이를 통해 필요한 서버의 수를 측정한다면, 쓰기 작업을
수행하는 `Command` 서버는 `최소 6대의 서버` 가 필요하며, `Query` 서버도 `최소 2대 서버` 가 필요하다.
